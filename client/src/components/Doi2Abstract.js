...
class Doi2Abstract extends Component {
  constructor(props) {
    super(props);
    let doiInUrl = '';
    if (props.location.pathname.startsWith(ABSTRACT)) {
      doiInUrl = props.location.pathname.substring(ABSTRACT.length);
    }
    this.state = {
      value: doiInUrl
    };
    ...
    if(url) {
      fetch(getDomain() + url + '?id=' + idToSend)
        ...
        .then(data => {
          ...
          if (changeBrowserURL) {
            this.props.history.push('/abstract/' + this.state.value);
          }
        }, data => {
          this.setState({
            error: data,
